                    ...
                    @if (Auth::check())
                        <el-dropdown class="relative ml-3">
                            <button
                                ...
                                <img src="{{ Auth::user()->avatar ? asset(Auth::user()->avatar) : asset('img/default-avatar.png') }}"
                                    alt="{{ Auth::user()->name }}"
                                    class="size-8 rounded-full outline -outline-offset-1 outline-white/10" />
                                <div class="ml-3 text-sm text-gray-300">{{ Auth::user()->name }}</div>
                            ...
                            <el-menu anchor="bottom end" popover
                                class="w-42 origin-top-right rounded-md bg-white py-1 shadow-lg outline-1 outline-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                                <a href="/profile"
                                ...
                                <form action="/logout" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full text-left cursor-pointer px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:outline-hidden">Sign
                                        out</button>
                                </form>
                            </el-menu>
                        </el-dropdown>
            ...
            @if (Auth::check())
                ...
                <div class="mt-3 space-y-1 px-2">
                    ...
                    <form action="/logout" method="POST">
                        @csrf
                        <button type="submit"
                            class="block w-full text-left cursor-pointer rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 hover:text-white">Sign
                            out</button>
                    </form>
                </div>
            @else
